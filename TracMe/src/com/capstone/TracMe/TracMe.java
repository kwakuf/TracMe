...
public class TracMe extends SlidingActivity implements OnClickListener {
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
	...
	public void onClick(View view) {
		if (view.getId() == R.id.buttonScan) {
		  if (valuesConfirmed == 0) {
		    ...
		    buttonPressed = "scan";
	      String inputStr = ((EditText)findViewById(R.id.editTextX)).getText().toString();
	      ...
	      inputStr = ((EditText)findViewById(R.id.editTextY)).getText().toString();
        if (inputStr.length() <= 0) {
        ...
        if (checkCell()) {
          ...
          .setMessage("This cell has already been sampled. Do you want to Redo this cell sample?")
          .setPositiveButton("Redo",
            ...
            public void onClick(DialogInterface dialog,
                    int which) {
                deleteCell();
                writeToConsole("Coordinate being sampled: " + prog.getGridX() + "x" + prog.getGridY());
                scan(0);
                dialog.dismiss();
            }
           })
		  ...
		  String inputStr = ((EditText)findViewById(R.id.editTextOutFile)).getText().toString();
		  if (inputStr.length() <= 0) {
		  ...
		  inputStr = ((EditText)findViewById(R.id.editTextAPTFile)).getText().toString();
      if (inputStr.length() <= 0) {
        ...
        aptablefile = new String(inputStr);
		  ...
		  inputStr = ((Spinner)findViewById(R.id.spinner1)).getSelectedItem().toString();
		  direction = new String(inputStr);
		  
		  inputStr = ((EditText)findViewById(R.id.editTextXaxis)).getText().toString();
      if (inputStr.length() <= 0) {
        alertError("Please enter a valid input for the X axis");
        return;
      }
      ...
      inputStr = ((EditText)findViewById(R.id.editTextYaxis)).getText().toString();
      if (inputStr.length() <= 0) {
        alertError("Please enter a valid input for the Y axis");
        return;
      }
      ...
      inputStr = ((EditText)findViewById(R.id.editTextNumSamples)).getText().toString();
      if (inputStr.length() <= 0) {
      ...
      try {
        ...
        return;
		  ...
		  //First we should check for errors and empty fields, if all of that is done, alert them to confirm values
		  dialog = new AlertDialog.Builder(this).setTitle("Confirm Values")
		                                        .setMessage("Are you sure you want to save these values? (They cannot be changed once set.)")
      ...
      dialog = new AlertDialog.Builder(this).setTitle("Save Results")
      .setMessage("Sampling finished. Do you want to save these results?")
      .setPositiveButton("Save",
          new DialogInterface.OnClickListener() {
        @Override
        public void onClick(DialogInterface dialog,
                int which) {
          prog.finishSampling("", ""); // Finish sampling will be in the save results onclick event
          writeToConsole("Sample saved at " + outputfile + ".txt");
          dialog.dismiss();
        }
       })
      .setNeutralButton("Cancel", null)
      ...
      if(gps.canGetLocation()){
        ...
        dialog = new AlertDialog.Builder(this).setTitle("GPS").setMessage(gpsString).setNeutralButton("Close", null).show();  
      }else{
          // can't get location
	...
	/**
	 * This function makes these fields no longer changeable and sets confirm to true
	 */
    ...
    writeToConsole("Grid dimensions set to: " + prog.getGridSizeX() + "x" + prog.getGridSizeY());
    writeToConsole("Number of samples per cell: " + numSamples);
	  valuesConfirmed = 1;
	}
	
	public void alertError(String msg) {
	  dialog = new AlertDialog.Builder(this).setTitle("Error").setMessage(msg).setNeutralButton("Close", null).show();
	}
    ...
    for( int cellCheck = 0; cellCheck < prog.getSamples().size(); cellCheck++ )
    {
      if ( prog.getGridX() == prog.getSamples().get( cellCheck ).getLoc().x
        && prog.getGridY() == prog.getSamples().get( cellCheck ).getLoc().y ) {
        return true;  
      }
    }
    return false;
	...
	public ArrayList< AccessPoint > scan(int ndx) {
      ...
      dialog = new AlertDialog.Builder(this).setTitle("TracMe").setMessage("Scanning").show();
      //TODO add a progress bar to this alert
	  }
	  if (ndx != numSamples) { // This index will be the variable
	    dialog.setMessage("Scanning: " + ndx + "/" + numSamples + " Complete" );
	    wifi.startScan();
	    index = ndx;
	  }
	    ...
	    dialog.setMessage("Scanning: " + ndx + "/" + numSamples + " Complete" );
	    dialog.dismiss();
	    writeToConsole( "Number of samples done: "+ prog.getSamples().size() );
